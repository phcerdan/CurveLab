...
int fdct_wrapping_sepangle(double XL1, double XL2, int nbangle, CpxOffMat& Xhgh,
                           std::vector<CpxNumMat>& csc);
int fdct_wrapping_wavelet(CpxOffMat& Xhgh, std::vector<CpxNumMat>& csc);

//-------------------------------------------------------------------
int fdct_wrapping(int N1, int N2, int nbscales, int nbangles_coarse,
                  int allcurvelets, CpxNumMat& x,
                  std::vector<std::vector<CpxNumMat> >& c) {
  //---------------------------------------------
  ...
  for (int j = 0; j < N2; j++)
    for (int i = 0; i < N1; i++) T(i, j) /= sqrtprod;
  CpxOffMat O(N1, N2);
  ...
  if (allcurvelets == 1) {
    ...
    for (int j = -XF2; j < -XF2 + XS2; j++)
      for (int i = -XF1; i < -XF1 + XS1; i++) X(i, j) = O(t1(i), t2(j));
    DblOffMat lowpass(XS1, XS2);
    fdct_wrapping_lowpasscompute(XL1, XL2,
                                 lowpass);  // compute the low pass filter
    for (int j = -XF2; j < -XF2 + XS2; j++)
      for (int i = -XF1; i < -XF1 + XS1; i++) X(i, j) *= lowpass(i, j);
  } else {
    //--------------------------
    X = O;
  }
  ...
  for (int sc = nbscales - 1; sc > 0; sc--) {
    ...
    CpxOffMat Xhgh(X);
    for (int j = -XF2n; j < -XF2n + XS2n; j++)
      for (int i = -XF1n; i < -XF1n + XS1n; i++) Xhgh(i, j) *= hghpass(i, j);
...
int fdct_wrapping_sepangle(double XL1, double XL2, int nbangle, CpxOffMat& Xhgh,
  ...
  for (int qi = 0; qi < nbquadrants; qi++) {
    ...
    fdct_wrapping_rotate_forward(q, XL1b, XL2b, XL1, XL2);
    XL1 = abs(XL1);
    XL2 = abs(XL2);
    ...
    for (int w = nd - 1; w >= 0; w--) {
      ...
      for (int xcur = xf; xcur < xe; xcur++) {  // for each layer
        ...
        for (int ycur = yfm; ycur <= yto; ycur++) {
          ...
          if (thtcur < thtm) {
            double l, r;
            fdct_wrapping_window((thtcur - thts) / (thtm - thts), l, r);
            wtht = l;
          } else {
            double l, r;
            fdct_wrapping_window((thtcur - thtm) / (thte - thtm), l, r);
            wtht = r;
          }
          double pou = wtht;
      ...
      // IFFT
      {
        ...
        for (int j = 0; j < yn; j++)
          for (int i = 0; i < xn; i++) tpdata(i, j) /= sqrtprod;
        // store
        csc[wcnt] = tpdata;
      }
