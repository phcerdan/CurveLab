...
//  ac -- ac==1 use curvelet at the finest level, ac==0, use wavelet at the
//  finest level x -- N1 by N2 matrix stored in CpxNumMat class
// OUTPUTS:
...
//  nbangles_coarse -- the number of angles in the 2nd coarest scale
//  ac -- ac==1 use curvelet at the finest level, ac==0, use wavelet at the
//  finest level c -- the curvelet coefficients data structure coeffient at
//  scale s, wedge w and indices (i,j) is accessed by c[s][w](i,j).
// OUTPUTS:
...
//  finest level
// OUTPUTS:
//  sx -- for scale s and wege w, sx[s][w] is the first-cooridnate spacing of
//  curvelet grid of scale s and wedge w sy -- for scale s and wege w, sy[s][w]
//  is the second-cooridnate spacing of curvelet grid of scale s and wedge w fx
//  -- for scale s and wege w, fx[s][w] is the first coordinate of the frequency
//  center of curvelet of scale s and wedge w fy -- for scale s and wege w,
//  fy[s][w] is the second coordinate of the frequency center of curvelet of
//  scale s and wedge w nx -- for scale s and wege w, nx[s][w] is the first
//  dimension of the curvelet matrix of scale s and wedge w ny -- for scale s
//  and wege w, ny[s][w] is the second dimension of the curvelet matrix of scale
//  s and wedge w
// COMMENTS:
//  In computing sx and sy, we assume that the input grid are Nyquist sampling
//  grid of domain [-1/2,1/2) by [-1/2,1/2) with the center sampling point at
//  origin (0,0). Therefore, for any scale s, and wedge w, nx[s][w] * sx[s][w] =
//  1 and ny[s][w] * sy[s][w] = 1.
