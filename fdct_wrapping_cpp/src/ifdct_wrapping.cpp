...
int ifdct_wrapping(int N1, int N2, int nbscales, int nbangles_coarse,
  ...
  for (int sc = nbscales - 1; sc > 0; sc--) {
    ...
    for (int j = G.t(); j < G.t() + G.n(); j++)
      for (int i = G.s(); i < G.s() + G.m(); i++) X(i, j) += G(i, j);
    XL1 = XL1 / 2;
    XL2 = XL2 / 2;
    fdct_wrapping_rangecompute(XL1, XL2, XS1, XS2, XF1, XF2, XR1, XR2);
  }
  for (int j = -XF2; j < -XF2 + XS2; j++)
    for (int i = -XF1; i < -XF1 + XS1; i++) X(i, j) += Xhghs[0](i, j);
  // fold
  CpxOffMat O(N1, N2);
  if (allcurvelets == 1) {
    double XL1 = 4.0 * N1 / 3.0;
    ...
    for (int i = -XF2; i < -XF2 + XS2; i++)
      if (i < -N2 / 2)
        ...
        t2(i) = i;
    for (int j = -XF2; j < -XF2 + XS2; j++)
      for (int i = -XF1; i < -XF1 + XS1; i++) O(t1(i), t2(j)) += X(i, j);
  } else {
    O = X;
  }
  //------------------------------------------------------------
...
int fdct_wrapping_invwavelet(vector<CpxNumMat>& csc, CpxOffMat& Xhgh) {
  ...
  CpxNumMat T(C);  // CpxNumMat T(N1, N2);  fdct_wrapping_ifftshift(N1, N2, F1,
                   // F2, C, T);
