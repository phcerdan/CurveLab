...
int fdct_wrapping_param(int N1, int N2, int nbscales, int nbangles_coarse,
  ...
  if (allcurvelets == 1) {
    ...
    for (int sc = nbscales - 1; sc > 0; sc--) {
      fdct_wrapping_param_sepangle(XL1, XL2, nbangles[sc], sx[sc], sy[sc],
                                   fx[sc], fy[sc], nx[sc], ny[sc]);
      XL1 /= 2;
      XL2 /= 2;
    }
    // coarsest level
    ...
    fdct_wrapping_param_wavelet(XS1, XS2, sx[0], sy[0], fx[0], fy[0], nx[0],
                                ny[0]);
  } else {
    // nbangles
    nbangles[0] = 1;
    ...
    // top level
    fdct_wrapping_param_wavelet(N1, N2, sx[nbscales - 1], sy[nbscales - 1],
                                fx[nbscales - 1], fy[nbscales - 1],
                                nx[nbscales - 1], ny[nbscales - 1]);
    // next levels
...
int fdct_wrapping_param_sepangle(double XL1, double XL2, int nbangle,
  ...
  for (int qi = 0; qi < nbquadrants; qi++) {
    ...
    for (int w = nd - 1; w >= 0; w--) {
      ...
      // fx,fy
      tx = XR1 / 2;
      ty = (-XR2 + (w + 0.5) * XW2) / 2;  // center, NOTE: need /2
      fdct_wrapping_rotate_backward(q, tx, ty, fx[wcnt], fy[wcnt]);
      // sx, sy;
      tx = 1.0 / xn;
      ty = 1.0 / yn;
      fdct_wrapping_rotate_backward(q, xn, yn, nx[wcnt], ny[wcnt]);
      nx[wcnt] = abs(nx[wcnt]);
      ny[wcnt] = abs(ny[wcnt]);
      fdct_wrapping_rotate_backward(q, tx, ty, sx[wcnt], sy[wcnt]);
      sx[wcnt] = abs(sx[wcnt]);
      sy[wcnt] = abs(sy[wcnt]);
      wcnt++;
    }
