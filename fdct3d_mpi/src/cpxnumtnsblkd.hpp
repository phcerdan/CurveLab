...
class CpxNumTnsBlkd {
 protected:  // PaData _padata;
  ...
  IntNumTns
      _sizes;  // for each block, the size (in terms of the number of doubles)
  BolNumTns _exists;  // for each box, whether exisits on thiis processor
  NumTns<CpxNumTns> _blocks;  // NumTns<CpxNumTns> _blocks; //a tensor of
                              // tensors
 ...
 public:
  ...
  int expand(
      BolNumTns& newexists);  // no communication, just expand with zero data
  ...
  CpxNumTns& block(int i, int j, int k) {
    ...
    assert(tmp.m() == _b && tmp.n() == _b && tmp.p() == _b);
