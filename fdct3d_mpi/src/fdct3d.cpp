...
int fdct3d_partition_cpxcrvletprtd(int N1, int N2, int N3, int nbscales,
  ...
  for (int s = 1; s < nbscales - 1; s++) {
    ...
    int face;
    int wcnt = 0;
    int wpp = (nd * nd * nf) / mpisize;
    if (wpp * mpisize < nd * nd * nf) wpp++;
    // face 0, x y z
    face = 0;
    for (int h = 0; h < nd; h++) {
      for (int g = 0; g < nd; g++) {
    ...
    for (int xcur = -F1; xcur <= F1; xcur++)
      for (int ycur = -F2; ycur <= F2; ycur++)
        for (int zcur = -F3; zcur <= F3; zcur++) {
          int bi, bj, bk;
          int oi, oj, ok;
          fdct3d_position_aux(N1, N2, N3, b, xcur, ycur, zcur, bi, bj, bk, oi,
                              oj, ok);
  ...
  for (int h = 0; h < nd; h++) {
    for (int g = 0; g < nd; g++) {
      if (crvowners[wcnt] == mpirank) {
        ...
        // int xh = xn/2;		  int yh = yn/2;		  int zh =
        // zn/2;
        double R21 = R2 / R1;
        double R31 = R3 / R1;
...
int fdct3d_dependency(int N1, int N2, int N3, int b, int nbscales,
  ...
  int L = nbscales;
  {
    ...
    fdct3d_dependency_center(N1, N2, N3, b, L1, L2, L3, crvowners[s],
                             tnsexists);
  }
  for (int s = 1; s < nbscales - 1; s++) {
    double L1 = 2.0 * N1 / 3.0 / pow2(L - 2 - s);
    ...
    double L3 = 2.0 * N3 / 3.0 / pow2(L - 2 - s);
    int nd = nbdstz_coarse * pow2(s / 2);
    fdct3d_dependency_angles(N1, N2, N3, b, L1, L2, L3, nd, crvowners[s],
                             tnsexists);
  }
  return 0;
}
...
int fdct3d_fft(CpxNumTnsBlkd& W) {
  // LEXING: entering with z slices, exiting with y slices, and the frequency is
  // centered
  time_t tm0, tm1;
  tm0 = time(NULL);
  ...
  // copy w's z slices into (x,y) cpxnummat and perform fft on each of them,
  // then put back
  fftwnd_plan pxyf =
      fftw2d_create_plan(N2, N1, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  for (int k = 0; k < g; k++) {
    int kstt = k * b;
  ...
  fftwnd_destroy_plan(pxyf);
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  fft compute1 %f\n",
  ...
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  fft scatter %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // shift w's owner to y slices
  iC(W.shift(newtnsowners));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  fft shift %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // discard w's nonowners
  iC(W.discard());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  fft discard %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;

  // copy w's data into (z) cpxnumvec and perform fft on each of them, then put
  // back
  fftw_plan pzf =
      fftw_create_plan(N3, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  for (int i = 0; i < e; i++)
    for (int j = 0; j < f; j++) {
      int istt = i * b;
      int jstt = j * b;
  ...
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  fft compute2 %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
...
int fdct3d_ifft(CpxNumTnsBlkd& W) {
  // LEXING: entering with y slices, exiting with z slices, and the frequency is
  // centered
  time_t tm0, tm1;
  tm0 = time(NULL);
  ...
  // copy w's data into (z) cpxnumvec and perform ifft on each of them, then put
  // back
  fftw_plan pzb =
      fftw_create_plan(N3, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  for (int i = 0; i < e; i++)
    for (int j = 0; j < f; j++) {
      int istt = i * b;
      int jstt = j * b;
  ...
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  ifft compute1 %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  iC(W.scatter(newtnsexists));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  ifft scatter %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // shift w's onwer to z slices
  iC(W.shift(newtnsowners));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  ifft shift %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // discard w's nonowners
  iC(W.discard());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "  ifft discard %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;

  // copy w's data into (x,y) cpxnummat and perform ifft on each of them, then
  // put back
  fftwnd_plan pxyb =
      fftw2d_create_plan(N2, N1, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  for (int k = 0; k < g; k++) {
    int kstt = k * b;
