...
//------------------------------------------------------------------------------------
int fdct3d_inverse_center(int N1, int N2, int N3, int b, double L1, double L2,
                          double L3, int s, CpxCrvletPrtd& C,
                          CpxNumTnsBlkd& W) {
  int mpirank;
...
//------------------------------------------------------------------------------------
int fdct3d_inverse_angles(int N1, int N2, int N3, int b, double L1, double L2,
                          double L3, int s, int nd, CpxCrvletPrtd& C,
                          CpxNumTnsBlkd& W) {
  int mpirank;
  MPI_Comm_rank(MPI_COMM_WORLD, &mpirank);
  int mpisize;
...
int fdct3d_inverse(int N1, int N2, int N3, int nbscales, int nbdstz_coarse,
                   CpxCrvletPrtd& C, CpxNumTnsBlkd& W, CpxNumTnsBlkd& X) {
  ...
  iC(MPI_Barrier(
      MPI_COMM_WORLD));  // iC( PetscPrintf(MPI_COMM_WORLD, "%d inverse 0\n",
  ...
  iC(X.scatter(newtnsexists));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x scatter %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // shift x's owner to z slices
  iC(X.shift(newtnsowners));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x shift %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // discard x's nonowners
  iC(X.discard());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x discard %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;

  // ifft
  iC(fdct3d_fft(X));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x fft %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  fdct3d_partition_cpxcrvletprtd(N1, N2, N3, nbscales, nbdstz_coarse,
  ...
  // //LEXING: might not be necessary

  // find out the required blocks from x for each processor
  fdct3d_dependency(N1, N2, N3, b, nbscales, nbdstz_coarse, newcrvowners,
                    newtnsexists);

  // expand x according to c's request
  iC(X.expand(newtnsexists));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x expand %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv c compute %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;

  // combine x
  iC(X.combine());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x combine %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  iC(fdct3d_ifft(X));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "inv x ifft %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // done
