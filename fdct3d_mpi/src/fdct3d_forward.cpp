...
//------------------------------------------------------------------------------------
int fdct3d_forward_center(int N1, int N2, int N3, int b, double L1, double L2,
                          double L3, int s, CpxNumTnsBlkd& W,
                          CpxCrvletPrtd& C) {
  int mpirank;
  MPI_Comm_rank(MPI_COMM_WORLD, &mpirank);
  std::vector<std::vector<int> >& Cowners = C.owners();
  ...
  if (Cowners[0][0] == mpirank) {
    ...
    for (int i = -S1 / 2; i < -S1 / 2 + S1; i++)
      for (int j = -S2 / 2; j < -S2 / 2 + S2; j++)
        for (int k = -S3 / 2; k < -S3 / 2 + S3; k++) {
          int bi, bj, bk;
          int oi, oj, ok;
          fdct3d_position_aux(N1, N2, N3, b, i, j, k, bi, bj, bk, oi, oj, ok);
    ...
    fftwnd_plan p = fftw3d_create_plan(S3, S2, S1, FFTW_BACKWARD,
...
//------------------------------------------------------------------------------------
int fdct3d_forward_angles(int N1, int N2, int N3, int b, double L1, double L2,
                          double L3, int s, int nd, CpxNumTnsBlkd& W,
                          CpxCrvletPrtd& C) {
  int mpirank;
  MPI_Comm_rank(MPI_COMM_WORLD, &mpirank);
  int mpisize;
  ...
  std::vector<std::vector<int> >& Cowners = C.owners();
  std::vector<int>& crvowners =
      Cowners[s];  // LEXING: the owner information for wedges in scale s
        ...
        for (int xcur = (int)ceil(xs); xcur < xe; xcur++) {
          ...
          for (int ycur = yfm; ycur <= yto; ycur++)
            for (int zcur = zfm; zcur <= zto; zcur++) {
              ...
              int bi, bj, bk;
              int oi, oj, ok;
              fdct3d_position_aux(N1, N2, N3, b, xcur, ycur, zcur, bi, bj, bk,
                                  oi, oj, ok);
              CpxNumTns& Wblk = W.block(bi, bj, bk);
              wpdata(tmpx, tmpy, tmpz) =
                  Wblk(oi, oj, ok) * bb *
                  sqrt(1.0 - ss * ss);  // pou1(xcur)*pou2(ycur)*pou3(zcur);
...
int fdct3d_forward(int N1, int N2, int N3, int nbscales, int nbdstz_coarse,
                   CpxNumTnsBlkd& X, CpxCrvletPrtd& C, CpxNumTnsBlkd& W) {
  ...
  iC(MPI_Barrier(
      MPI_COMM_WORLD));  // iC( PetscPrintf(MPI_COMM_SELF, "%d forward 0\n",
  ...
  // setup c 1,2,3, 6*np/8 processors are computing. 0 processor contains also
  // the center wedge
  std::vector<std::vector<bool> > newcrvexists;
  std::vector<std::vector<int> > newcrvowners;
  iC(fdct3d_partition_cpxcrvletprtd(N1, N2, N3, nbscales, nbdstz_coarse,
                                    newcrvexists, newcrvowners));
  std::vector<std::vector<double> > fxs, fys, fzs;
  std::vector<std::vector<int> > nxs, nys, nzs;
  iC(fdct3d_param(N1, N2, N3, nbscales, nbdstz_coarse, fxs, fys, fzs, nxs, nys,
                  nzs));
  nxs.resize(L - 1);
  nys.resize(L - 1);
  nzs.resize(L - 1);  // remove the last level
  iC(C.setup(nxs, nys, nzs, newcrvowners));

  // find out the required blocks from w for each processor
  iC(fdct3d_dependency(N1, N2, N3, b, nbscales, nbdstz_coarse, newcrvowners,
                       newtnsexists));
  // scatter w according to c's request
  iC(W.scatter(newtnsexists));
  iC(W.check());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "fwd w scatter %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;

  // compute c (standard computation, since data is local already)
  {
    int s = 0;
    double L1 = 2.0 * N1 / 3.0 / pow2(L - 2 - s);
    double L2 = 2.0 * N2 / 3.0 / pow2(L - 2 - s);
    double L3 = 2.0 * N3 / 3.0 / pow2(L - 2 - s);
  ...
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "fwd c compute %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  iC(W.check());
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "fwd w discard %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  ...
  DblOffVec big1(N1);
  fdct3d_lowpass(2.0 * N1 / 3, big1);
  DblOffVec big2(N2);
  fdct3d_lowpass(2.0 * N2 / 3, big2);
  DblOffVec big3(N3);
  fdct3d_lowpass(2.0 * N3 / 3, big3);
          ...
          for (int ioff = 0; ioff < b; ioff++)
            for (int joff = 0; joff < b; joff++)
              for (int koff = 0; koff < b; koff++) {
                double pou =
                    big1(ioff + istt) * big2(joff + jstt) * big3(koff + kstt);
  ...
  iC(fdct3d_ifft(W));
  iC(MPI_Barrier(MPI_COMM_WORLD));
  tm1 = time(NULL);  // iC( PetscPrintf(MPI_COMM_WORLD, "fwd w ifft %f\n",
                     // difftime(tm1,tm0)) );  tm0 = tm1;
  // done
  // tm1 = time(NULL);  //iC( PetscPrintf(MPI_COMM_WORLD, "%f   ",
  // difftime(tm1,tm0)) );  tm0 = tm1; iC( PetscPrintf(MPI_COMM_WORLD, "forward
  // 3\n") );  iC( MPI_Barrier(MPI_COMM_WORLD) );
