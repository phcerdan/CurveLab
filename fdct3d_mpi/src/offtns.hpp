...
class OffTns {
 ...
 public:
  OffTns(int m = 0, int n = 0, int p = 0)
  ...
  const F& operator()(int i, int j, int k) const {
    assert(i >= _s && i < _m + _s && j >= _t && j < _n + _t && k >= _u &&
           k < _p + _u);
...
inline void setvalue(OffTns<F>& T, F val) {
  for (int i = T.s(); i < T.m() + T.s(); i++)
    for (int j = T.t(); j < T.n() + T.t(); j++)
      for (int k = T.u(); k < T.p() + T.u(); k++) T(i, j, k) = val;
}
// template <class F> inline void clear(OffTns<F>& T){  memset(T.data(), 0,
