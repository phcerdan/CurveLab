...
int main(int argc, char** argv) {
  PetscInitialize(&argc, &argv, "options",
                  NULL);  // PetscTruth flg = PETSC_FALSE;
  srand48(0);
  ...
  int m;
  iC(PetscOptionsGetInt("", "-m", &m, &flg));
  iA(flg == PETSC_TRUE);
  int n;
  iC(PetscOptionsGetInt("", "-n", &n, &flg));
  iA(flg == PETSC_TRUE);
  int p;
  iC(PetscOptionsGetInt("", "-p", &p, &flg));
  iA(flg == PETSC_TRUE);
  int b;
  iC(PetscOptionsGetInt("", "-b", &b, &flg));
  iA(flg == PETSC_TRUE);
  int nbscales;
  iC(PetscOptionsGetInt("", "-nbscales", &nbscales, &flg));
  iA(flg == PETSC_TRUE);
  int nbdstz_coarse;
  iC(PetscOptionsGetInt("", "-nbdstz_coarse", &nbdstz_coarse, &flg));
  iA(flg == PETSC_TRUE);
          ...
          for (int ioff = 0; ioff < b; ioff++)
            for (int joff = 0; joff < b; joff++)
              for (int koff = 0; koff < b; koff++) {
                Xblk(ioff, joff, koff) =
                    cpx(drand48(), 0);  // cpx(drand48(), drand48());
              }
        }
      }
  ...
  tm1 = time(NULL);
  iC(PetscPrintf(MPI_COMM_WORLD, "FORWARD %e\n", difftime(tm1, tm0)));
  tm0 = tm1;
  ...
  tm1 = time(NULL);
  iC(PetscPrintf(MPI_COMM_WORLD, "INVERSE %e\n", difftime(tm1, tm0)));
  tm0 = tm1;
  ...
  for (int i = 0; i < e; i++)
    for (int j = 0; j < f; j++)
      for (int k = 0; k < g; k++) {
        if (X.owners()(i, j, k) == mpirank) {
          iA(Y.owners()(i, j, k) ==
             mpirank);  // CHECK THEY HAVE THE SAME DISTRIBUTION
          CpxNumTns& Xblk = X.block(i, j, k);
          CpxNumTns& Yblk = Y.block(i, j, k);
          for (int ioff = 0; ioff < b; ioff++)
            for (int joff = 0; joff < b; joff++)
              for (int koff = 0; koff < b; koff++) {
