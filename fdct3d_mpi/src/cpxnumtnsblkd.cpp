...
double CpxNumTnsBlkd::globalenergy() {
  ...
  for (int i = 0; i < e(); i++)
    for (int j = 0; j < f(); j++)
      for (int k = 0; k < g(); k++)
        if (_owners(i, j, k) == mpirank()) lclsum += energy(_blocks(i, j, k));
  double glbsum = 0;
  iC(MPI_Reduce((void*)(&lclsum), (void*)(&glbsum), 1, MPI_DOUBLE, MPI_SUM, 0,
                PETSC_COMM_WORLD));
  return glbsum;
}
...
int CpxNumTnsBlkd::setup(int m, int n, int p, int b, IntNumTns& owners) {
  ...
  for (int k = 0; k < g(); k++)
    for (int j = 0; j < f(); j++)
      for (int i = 0; i < e(); i++) _sizes(i, j, k) = blksz;
  // exists + blocks
...
int CpxNumTnsBlkd::scatter(BolNumTns& newexists) {
  ...
  int blksz = _b * _b * _b * C2D;
  IS lclis;
  iC(ISCreateBlock(PETSC_COMM_SELF, blksz, lclbid.size(), &(lclbid[0]),
                   &lclis));
  IS glbis;
  iC(ISCreateBlock(PETSC_COMM_WORLD, blksz, glbbid.size(), &(glbbid[0]),
                   &glbis));
  lclbid.clear();
  glbbid.clear();
  ...
  IS glbis;  iC( ISCreateGeneral(PETSC_COMM_WORLD, l2gmap.size(), &(l2gmap[0]),
  ...
  */
  ...
  // 3. vec scatter
  VecScatter sc;
  iC(VecScatterCreate(glbvec, glbis, lclvec, lclis, &sc));
  iC(ISDestroy(lclis));
  iC(ISDestroy(glbis));  // SAVE SPACE
    ...
    for (int j = 0; j < f(); j++)
      for (int i = 0; i < e(); i++) {
        ...
        if (pi == mpirank()) {
          ...
          for (int g = 0; g < _sizes(i, j, k); g++) {
            tmpptr[g] += *glbptr;
            glbptr++;  // LEXING: += is very important
          }
        }
      }
  iC(VecRestoreArray(glbvec, &glbarr));
