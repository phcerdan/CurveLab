  ...
  for (int s = 0; s < c.size(); s++)
    for (int w = 0; w < c[s].size(); w++)
      if (_exists[s][w] == true) {
        ...
        iA(tmp.m() == _nx[s][w] && tmp.n() == _ny[s][w] &&
           tmp.p() == _nz[s][w]);
      }
  return 0;
}
...
int CpxCrvletPrtd::setup(std::vector<std::vector<int> > nx, std::vector<std::vector<int> > ny,
  ...
  for (int s = 0; s < c.size(); s++) {
    _exists[s].resize(c[s].size());
    _blocks[s].resize(c[s].size());
    for (int w = 0; w < c[s].size(); w++)
...
int CpxCrvletPrtd::scatter(std::vector<std::vector<bool> >& newexists) {
  ...
  IS lclis;
  iC(ISCreateStride(PETSC_COMM_SELF, l2gmap.size(), 0, 1, &lclis));
  IS glbis;
  ...
  Vec lclvec;
  iC(VecCreateSeq(PETSC_COMM_SELF, lclsum, &lclvec));
