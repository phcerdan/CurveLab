 ...
 public:
  ...
  OffMat(const OffMat& C) : _m(C._m), _n(C._n), _s(C._s), _t(C._t) {
    if (_m > 0 && _n > 0) {
      ...
      _data = NULL;
    if (_m > 0 && _n > 0) {
      memcpy(_data, C._data, _m * _n * sizeof(F));
    }
  }
  ~OffMat() {
    if (_m > 0 && _n > 0) {
  ...
  const F& operator()(
      int i, int j) const {  // assert( i>=_s && i<_m+_s && j>=_t && j<_n+_t );
    return _data[(i - _s) + (j - _t) * _m];
  }
  F& operator()(int i,
                int j) {  // assert( i>=_s && i<_m+_s && j>=_t && j<_n+_t );
    return _data[(i - _s) + (j - _t) * _m];
  }
...
// SET VALUE
template <class F>
inline void setvalue(OffMat<F>& mat, F val) {
  for (int i = mat.s(); i < mat.s() + mat.m(); i++)
    for (int j = mat.t(); j < mat.t() + mat.n(); j++) mat(i, j) = val;
}
// CLEAR
template <class F>
inline void clear(OffMat<F>& M) {
