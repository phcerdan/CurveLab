      ...
      for (int xcur = (int)ceil(xs); xcur < xe; xcur++) {
        ...
        for (int ycur = yfm; ycur <= yto; ycur++)
          for (int zcur = zfm; zcur <= zto; zcur++) {
            ...
            // W.block(bi,bj,bk); Wblk(oi,oj,ok) += wpdata(tmpx,tmpy,tmpz)  * bb
            // * sqrt(1.0-ss*ss);
            O(xcur, ycur, zcur) +=
...
int fdct3d_inverse_wavelet(double L1, double L2, double L3, int s, CpxNumTns& W,
                           CpxOffTns& O) {
  /// std::vector<CpxNumTns>& csc = C[s];
...
int fdct3d_inverse(int N1, int N2, int N3, int nbscales, int nbdstz_coarse,
  ...
  for (int i = 0; i < N1; i++)
    for (int j = 0; j < N2; j++)
      for (int k = 0; k < N3; k++) T(i, j, k) /= sqrtprod;
  // tm1 = time(NULL);  std::cout<<"ifft+shift "<<difftime(tm1,tm0)<<std::endl;  tm0 =
  // tm1;
