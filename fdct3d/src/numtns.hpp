...
class NumTns {
 ...
 public:
  ...
  const F& operator()(int i, int j, int k)
      const {  // assert( i>=0 && i<_m && j>=0 && j<_n && k>=0 && k<_p );
    return _data[i + j * _m + k * _m * _n];
  }
  F& operator()(
      int i, int j,
      int k) {  // assert( i>=0 && i<_m && j>=0 && j<_n && k>=0 && k<_p );
    return _data[i + j * _m + k * _m * _n];
  }
...
inline void setvalue(NumTns<F>& T, F val) {
  for (int i = 0; i < T.m(); i++)
    for (int j = 0; j < T.n(); j++)
      for (int k = 0; k < T.p(); k++) T(i, j, k) = val;
}
template <class F>
inline void clear(NumTns<F>& T) {
