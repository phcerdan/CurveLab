...
inline int fdct3d_fftshift(int N1, int N2, int N3, CpxNumTns& T, CpxOffTns& O) {
  ...
  IntOffVec p3(N3);
  for (int i = -F3; i < -F3 + N3; i++) p3(i) = (i + N3) % N3;
  for (int i = -F1; i < -F1 + N1; i++)
    for (int j = -F2; j < -F2 + N2; j++)
      for (int k = -F3; k < -F3 + N3; k++) O(i, j, k) = T(p1(i), p2(j), p3(k));
  return 0;
}

inline int fdct3d_ifftshift(int N1, int N2, int N3, CpxOffTns& O,
                            CpxNumTns& T) {
  int F1 = N1 / 2;
  int F2 = N2 / 2;
  int F3 = N3 / 2;
  ...
  for (int i = -F1; i < -F1 + N1; i++)
    for (int j = -F2; j < -F2 + N2; j++)
      for (int k = -F3; k < -F3 + N3; k++) T(p1(i), p2(j), p3(k)) = O(i, j, k);
  return 0;
}
...
template <class F>
inline int fdct3d_rotate_forward(int f, F XL1, F XL2, F XL3, F& TL1, F& TL2,
                                 F& TL3) {
  if (f == 0) {
    TL1 = XL1;
    TL2 = XL2;
...
template <class F>
inline int fdct3d_rotate_backward(int f, F XL1, F XL2, F XL3, F& TL1, F& TL2,
                                  F& TL3) {
  if (f == 0) {
    TL1 = XL1;
    TL2 = XL2;
...
inline int fdct3d_rotate_forward(int f, CpxOffTns& X, CpxOffTns& T) {
  ...
  if (f == 0) {
    ...
    TS3 = XS3;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1;
         i++) {  // int ni = (-i<-XF1+XS1) ? -i : -i-XS1;
      for (int j = -XF2; j < -XF2 + XS2;
           j++) {  // int nj = (-j<-XF2+XS2) ? -j : -j-XS2;
        for (int k = -XF3; k < -XF3 + XS3;
             k++) {  // int nk = (-k<-XF3+XS3) ? -k : -k-XS3;
          T(-i, -j, -k) = X(i, j, k);
        }
      }
    }
    ...
    TS1 = XS2;
    TS2 = XS3;
    TS3 = XS1;  // XF1 = XF2; XF2 = XF3; XF3 = XF1;	 //TL1 = XL2; TL2 = XL3;
                // TL3 = XL1;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1; i++) {
      for (int j = -XF2; j < -XF2 + XS2; j++) {
    ...
    TS1 = XS2;
    TS2 = XS3;
    TS3 = XS1;  // XF1 = XF3; XF2 = XF2; XF3 = XF1;	 //TL1 = XL3; TL2 = XL2;
                // TL3 = XL1;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1;
         i++) {  // int ni = (-i<-XF1+XS1) ? -i : -i-XS1;
...
inline int fdct3d_rotate_backward(int f, CpxOffTns& X, CpxOffTns& T) {
  ...
  if (f == 0) {
    TS1 = XS1;
    TS2 = XS2;
    TS3 = XS3;  // XF1 = XF1;	 XF2 = XF2;	 XF3 = XF3;	 //TL1 = XL1;
                // TL2 = XL2;  TL3 = XL3;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1; i++) {
      for (int j = -XF2; j < -XF2 + XS2; j++) {
    ...
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1;
         i++) {  // int ni = (-i<-XF1+XS1) ? -i : -i-TS1;
      for (int j = -XF2; j < -XF2 + XS2;
           j++) {  // int nj = (-j<-XF2+XS2) ? -j : -j-TS2;
        for (int k = -XF3; k < -XF3 + XS3;
             k++) {  // int nk = (-k<-XF3+XS3) ? -k : -k-TS3;
          T(-i, -j, -k) = X(i, j, k);
        }
      }
    }
    ...
    TS1 = XS2;
    TS2 = XS3;
    TS3 = XS1;  // XF1 = XF2; XF2 = XF1; XF3 = XF3;	 //TL1 = XL2; TL2 = XL1;
                // TL3 = XL3;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1;
         i++) {  // int ni = (-i<-XF1+XS1) ? -i : -i-TS1;
    ...
    TS1 = XS3;
    TS2 = XS1;
    TS3 = XS2;  // XF1 = XF3; XF2 = XF1; XF3 = XF2;	 //TL1 = XL3; TL2 = XL1;
                // TL3 = XL2;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1; i++) {
      for (int j = -XF2; j < -XF2 + XS2; j++) {
    ...
    TS1 = XS3;
    TS2 = XS1;
    TS3 = XS2;  // XF1 = XF3; XF2 = XF2; XF3 = XF1;	 //TL1 = XL3; TL2 = XL2;
                // TL3 = XL1;
    T.resize(TS1, TS2, TS3);
    for (int i = -XF1; i < -XF1 + XS1;
         i++) {  // int ni = (-i<-XF1+XS1) ? -i : -i-TS1;
...
inline int fdct3d_globalpou_shape(double lt, double rt, double u, double v,
                                  double& s) {
  double t, a, b;
  t = (u - lt) / (rt - lt);
  if (t <= 0)
...
inline int fdct3d_globalpou(double tht, double phi, double buf, double& pou) {
  ...
  if (abs(tht) <= lt && abs(phi) <= lt)
    ...
    pc[1] = atan2(1, tp);
    double sa;
    fdct3d_globalpou_shape(lt, rt, pa[0], pa[1], sa);
    double sb;
    fdct3d_globalpou_shape(lt, rt, pb[0], pb[1], sb);
    double sc;
    fdct3d_globalpou_shape(lt, rt, pc[0], pc[1], sc);
 ...
 * The size of commonSize should be 4 * N_i / 3.0.
 ...
 * @param
 */
inline void homogeneous_size_coeff(int S1, int S2, int S3,
