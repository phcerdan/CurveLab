...
int fdct_usfft_fftL(int N1, int N2, CpxNumMat& x, CpxOffMat& O) {
  ...
  for (int i = 0; i < N1; i++)
    for (int j = 0; j < N2; j++) T(i, j) /= sqrtprod;
  // 3. fftshift
  O.resize(N1, N2);
...
int fdct_usfft_sepscale(int N1, int N2, int nbscales, int ac, CpxOffMat& O,
  ...
  if (ac == 1) {
    ...
    X.resize(XS1, XS2);
    for (int i = -XF1; i < -XF1 + XS1; i++)
      for (int j = -XF2; j < -XF2 + XS2; j++) X(i, j) = O(t1(i), t2(j));
    DblOffMat lowpass(XS1, XS2);
    fdct_usfft_lowpasscompute(XL1, XL2, lowpass);  // compute the low pass
                                                   // filter
    for (int i = -XF1; i < -XF1 + XS1; i++)
      for (int j = -XF2; j < -XF2 + XS2; j++) X(i, j) *= lowpass(i, j);
  } else {
    //--------------------------
    X = O;
  }
  ...
  for (int sc = nbscales - 1; sc > 0; sc--) {
    ...
    CpxOffMat Xhgh(X);
    for (int i = -XF1n; i < -XF1n + XS1n; i++)
      for (int j = -XF2n; j < -XF2n + XS2n;
           j++) {  // Xhgh(i,j).re *= hghpass(i,j);		  Xhgh(i,j).im *=
                   // hghpass(i,j);
    ...
    for (int i = -XF1n; i < -XF1n + XS1n; i++)
      for (int j = -XF2n; j < -XF2n + XS2n;
           j++) {  // Xlow(i,j).re = X(i,j).re * lowpass(i,j);		  Xlow(i,j).im
...
int fdct_usfft_sepangle(double XL1, double XL2, int nbangles, CpxOffMat& Xhgh,
  ...
  for (int qi = 0; qi < nbquadrants; qi++) {
    int q = qvec[qi];
    // rotate data
    fdct_usfft_rotate_forward(q, XL1b, XL2b, XL1, XL2);
    XL1 = abs(XL1);
    XL2 = abs(XL2);
    ...
    int XF1, XF2;
    double XR1, XR2;
    fdct_usfft_rangecompute(XL1, XL2, XS1, XS2, XF1, XF2, XR1,
                            XR2);  // double XW1 = XL1/nd;
    double XW2 = XL2 / nd;
    double xs = XR1 / 4;
    double xe = XR1;
    int xf = int(ceil(xs));
    ...
    for (int xcur = xf; xcur < xe; xcur++) {  // for each line
      ...
      for (int w = 0; w < nd; w++) {
        ...
        for (int yid = 0; yid < yn; yid++) {
          ...
          if (tmp < s2 * xcur) {  // below
            double l, r;
            fdct_usfft_window((tmp / xcur - s0) / (s2 - s0), l, r);
            pou = l;
          } else {
            double l, r;
            fdct_usfft_window((tmp / xcur - s2) / (s4 - s2), l, r);
            pou = r;
          }
          ysft(yid, w) = tmp;
          wsft(yid, w) = pou;
      ...
      CpxOffVec val(XS2);  ////CpxOffVec val(XS2+1, -XF2-1);  //value for the
                           ///current column, the first element is zero
      ...
      for (int w = 0; w < nd; w++)
        for (int yid = 0; yid < yn; yid++) tsc[w](tmpx, yid - yh) = res(yid, w);
    }  // each line
...
int fdct_usfft_ifftS(std::vector<CpxOffMat>& msc, std::vector<CpxNumMat>& csc) {
  ...
  for (int w = 0; w < msc.size(); w++) {
    ...
    if (mit != planmap.end()) {
      ...
      p = fftw2d_create_plan(yn, xn, FFTW_BACKWARD,
    ...
    for (int i = 0; i < xn; i++)
      for (int j = 0; j < yn; j++) tpdata(i, j) /= sqrtprod;
    // shift
    csc[w] = tpdata;  // csc[w].resize(xn,yn,-xh,-yh);
...
int fdct_usfft_wavelet(CpxOffMat& Xhgh, std::vector<CpxNumMat>& csc) {
  ...
  for (int i = 0; i < N1; i++)
    for (int j = 0; j < N2; j++) T(i, j) /= sqrtprod;
  csc[0] = T;  // csc[0].resize(N1, N2, -F1, -F2);  //fdct_usfft_fftshift(N1,
               // N2, F1, F2, T, csc[0]);
  return 0;
}
...
int fdct_usfft_1dinterp(DblNumMat& off, DblNumMat& wgt, CpxOffVec& val,
  ...
  if (off.n() <= 64) {  // SIMPLE INTERPOLATION
    ...
    if (bit != b1map.end()) {
      ...
      bp = fftw_create_plan(N, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
      b1map[N] = bp;
    }
    ...
    for (int k = 0; k < N; k++) tmp(k) /= scale;  // scaling
    ...
    if (bit != b1map.end()) {
      ...
      bp =
          fftw_create_plan(D * N, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
      b1map[D * N] = bp;
    }
    CpxNumVec tmp(N);
    ...
    fftw_one(fp, (fftw_complex*)tmp.data(),
             NULL);  // here tmp are the real frequencies
