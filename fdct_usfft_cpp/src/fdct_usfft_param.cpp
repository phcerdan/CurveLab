...
int fdct_usfft_param(int N1, int N2, int nbscales, int nbangles_coarse, int ac,
  ...
  if (ac == 1) {
    ...
    for (int sc = nbscales - 1; sc > 0; sc--) {
      fdct_usfft_param_sepangle(XL1, XL2, nbangles[sc], sx[sc], sy[sc], fx[sc],
                                fy[sc], nx[sc], ny[sc]);
      XL1 /= 2;
      XL2 /= 2;
    }
    ...
    fdct_usfft_param_wavelet(XS1, XS2, sx[0], sy[0], fx[0], fy[0], nx[0],
                             ny[0]);
  } else {
    // nbangles
    std::vector<int> nbangles(nbscales);
    ...
    // top level
    fdct_usfft_param_wavelet(N1, N2, sx[nbscales - 1], sy[nbscales - 1],
...
int fdct_usfft_param_sepangle(double XL1, double XL2, int nbangle,
  ...
  for (int w = nd - 1; w >= 0; w--) {
    ...
    nx[wcnt] = xn;
    ny[wcnt] = yn;
    sx[wcnt] = dbl2(1.0 / xn, 0);
    sy[wcnt] = dbl2(-s2 * 1.0 / yn, 1.0 / yn);
    wcnt++;
  }
  // 1.
  for (int w = nd - 1; w >= 0; w--) {
    ...
    nx[wcnt] = xn;
    ny[wcnt] = yn;
    sx[wcnt] = dbl2(1.0 / xn, s2 * 1.0 / xn);
    sy[wcnt] = dbl2(0, 1.0 / yn);
    wcnt++;
  }
  // 0.
  for (int w = nd - 1; w >= 0; w--) {
