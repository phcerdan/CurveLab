...
inline int fdct_usfft_fftshift(CpxNumMat& T, CpxOffMat& O) {
  ...
  for (int j = -F2; j < -F2 + N2; j++)
    for (int i = -F1; i < -F1 + N1; i++) O(i, j) = T(p1(i), p2(j));
  return 0;
}
...
inline int fdct_usfft_ifftshift(CpxOffMat& O, CpxNumMat& T) {
  ...
  for (int i = -F2; i < -F2 + N2; i++) p2(i) = (i + N2) % N2;
  for (int j = -F2; j < -F2 + N2; j++)
    for (int i = -F1; i < -F1 + N1; i++) T(p1(i), p2(j)) = O(i, j);
  return 0;
}
...
inline int fdct_usfft_rangecompute(double XL1, double XL2, int& XS1, int& XS2,
                                   int& XF1, int& XF2, double& XR1,
                                   double& XR2) {
  XS1 = 2 * int(floor(XL1 / 2)) + 1;
  XS2 = 2 * int(floor(XL2 / 2)) + 1;  // number of samples
  XF1 = int(floor(XL1 / 2));
  XF2 = int(floor(XL2 / 2));  // offset on either side
  XR1 = XL1 / 2;
  XR2 = XL2 / 2;
  return 0;
}

inline int fdct_usfft_lowpasscompute(double XL1, double XL2,
                                     DblOffMat& lowpass) {
  int XS1 = 2 * int(floor(XL1 / 2)) + 1;
  int XS2 = 2 * int(floor(XL2 / 2)) + 1;  // number of samples
  int XF1 = int(floor(XL1 / 2));
  int XF2 = int(floor(XL2 / 2));  // offset on either side
  double XR1 = XL1 / 2;
  double XR2 = XL2 / 2;
...
inline int fdct_usfft_rotate_forward(int f, double XL1, double XL2, double& TL1,
                                     double& TL2) {
  if (f == 0) {  // time 1
    TL1 = XL1;
    TL2 = XL2;
...
inline int fdct_usfft_rotate_backward(int f, double XL1, double XL2,
...
template <class F>  // inline int fdct_usfft_rotate_forward(int f, CpxOffMat& X,
                    // CpxOffMat& T)
                    inline int fdct_usfft_rotate_forward(int f, OffMat<F>& X,
                                                         OffMat<F>& T) {
  ...
  if (f == 0) {
    ...
    TS2 = XS2;
    TF1 = XF1;
    TF2 = XF2;  // TL1 = XL1;	 TL2 = XL2;
    T.resize(TS1, TS2);
    for (int j = -TF2; j < -TF2 + TS2; j++)
      for (int i = -TF1; i < -TF1 + TS1; i++) T(i, j) = X(i, j);
  } else if (f == 1) {
    TS1 = XS2;
    TS2 = XS1;
    TF1 = XF2;
    TF2 = XF1;  // TL1 = XL2;	 TL2 = XL1;
    T.resize(TS1, TS2);
    for (int j = -TF2; j < -TF2 + TS2; j++)
      for (int i = -TF1; i < -TF1 + TS1; i++) T(i, j) = X(-j, i);
  } else if (f == 2) {
    TS1 = XS1;
    TS2 = XS2;
    TF1 = XF1;
...
template <class F>  // inline int fdct_usfft_rotate_backward(int f, CpxOffMat&
                    // X, CpxOffMat& T)
                    inline int fdct_usfft_rotate_backward(int f, OffMat<F>& X,
                                                          OffMat<F>& T) {
  int XS1 = X.m();
  int XS2 = X.n();
  int XF1 = -X.s();
