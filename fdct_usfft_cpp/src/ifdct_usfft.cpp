...
int fdct_usfft_invsepangle(double XL1, double XL2, int nbangles,
  ...
  cpx* rdata = r.data();  // cpx* xdata = x.data();  cpx* ddata = d.data(); cpx*
                          // qdata = q.data();
  ...
  for (int qi = 0; qi < nbquadrants; qi++) {
    ...
    for (int xcur = xf; xcur < xe; xcur++) {  // for each line construct lambda
      ...
      if (bit != b1map.end()) {
        ...
        bp = fftw_create_plan(2 * N - 1, FFTW_BACKWARD,
                              FFTW_ESTIMATE | FFTW_IN_PLACE);
        b1map[2 * N - 1] = bp;
      }
      fftw_one(bp, (fftw_complex*)tt.data(), NULL);
      scale = sqrt(double(2 * N - 1));
      for (int k = 0; k < 2 * N - 1; k++) tt(k) /= scale;  // scaling
    }                                                      // each line
    wcnt += nd;
  }  // quadrant
...
int fdct_usfft_multToeplitz(double XL1, double XL2, int nbangles,
  ...
  for (int qi = 0; qi < nbquadrants; qi++) {
    ...
    XL1 = abs(XL1);
    XL2 = abs(XL2);
    fdct_usfft_rotate_forward(q, db, d);
    fdct_usfft_rotate_forward(q, zb, z);
    ...
    for (int xcur = xf; xcur < xe; xcur++) {  // for each line construct lambda
      ...
      if (fit != f1map.end()) {
        ...
        sfp = fftw_create_plan(N, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
        f1map[N] = sfp;
      }
      fftw_plan lfp = NULL;
      ...
      if (fit != f1map.end()) {
        ...
        lfp = fftw_create_plan(2 * N - 1, FFTW_FORWARD,
                               FFTW_ESTIMATE | FFTW_IN_PLACE);
        f1map[2 * N - 1] = lfp;
      }
      ...
      if (bit != b1map.end()) {
        ...
        sbp = fftw_create_plan(N, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
        b1map[N] = sbp;
      }
      fftw_plan lbp = NULL;
      ...
      if (bit != b1map.end()) {
        ...
        lbp = fftw_create_plan(2 * N - 1, FFTW_BACKWARD,
                               FFTW_ESTIMATE | FFTW_IN_PLACE);
        b1map[2 * N - 1] = lbp;
      }
