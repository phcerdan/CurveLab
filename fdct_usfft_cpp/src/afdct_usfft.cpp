...
int fdct_usfft_adjfftL(int N1, int N2, CpxOffMat& O, CpxNumMat& x) {
  ...
  x = T;  // x.resize(N1, N2, -F1, -F2);  //fdct_usfft_fftshift(N1, N2, F1, F2,
          // T, x);
  return 0;
}
...
int fdct_usfft_adjsepscale(int N1, int N2, int nbscales, int ac,
  ...
  for (int sc = nbscales - 1; sc > 0; sc--) {
    ...
    for (int i = G.s(); i < G.s() + G.m(); i++)
      for (int j = G.t(); j < G.t() + G.n(); j++) X(i, j) += G(i, j);
    XL1 = XL1 / 2;
    XL2 = XL2 / 2;
    fdct_usfft_rangecompute(XL1, XL2, XS1, XS2, XF1, XF2, XR1, XR2);
  }
  for (int i = -XF1; i < -XF1 + XS1; i++)
    for (int j = -XF2; j < -XF2 + XS2; j++) X(i, j) += Xhghs[0](i, j);
  ...
  if (ac == 1) {
    ...
    for (int i = -XF2; i < -XF2 + XS2; i++)
      if (i < -N2 / 2)
        ...
        t2(i) = i;
    for (int i = -XF1; i < -XF1 + XS1; i++)
      for (int j = -XF2; j < -XF2 + XS2; j++) O(t1(i), t2(j)) += X(i, j);
  } else {
    O = X;
  }
...
int fdct_usfft_adjifftS(std::vector<CpxNumMat>& csc, std::vector<CpxOffMat>& msc) {
  ...
  for (int w = 0; w < csc.size(); w++) {
    ...
    if (mit != planmap.end()) {
      ...
      p = fftw2d_create_plan(yn, xn, FFTW_FORWARD,
...
int fdct_usfft_adjwavelet(std::vector<CpxNumMat>& csc, CpxOffMat& Xhgh) {
  ...
  CpxNumMat T(C);  // ifftshift  //CpxNumMat T(N1, N2); fdct_usfft_ifftshift(N1,
                   // N2, F1, F2, csc[0], T);
  // fft
  fftwnd_plan p =
...
int fdct_usfft_adj1dinterp(DblNumMat& off, DblNumMat& wgt, CpxNumMat& res,
  ...
  if (off.n() <= 64) {  // SIMPLE INTERPOLATION
    ...
    if (fit != f1map.end()) {
      ...
      fp = fftw_create_plan(D * N, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
      f1map[D * N] = fp;
    }
